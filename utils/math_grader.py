...
def hierarchy_format_reward(text):
    reward = 0.0

    # Extract INSTR and EXEC blocks
    instr = re.findall(r"<\|instruction\|>\s*Step\s*(\d+):", text, re.IGNORECASE)
    execs = re.findall(r"<\|execution\|>\s*Step\s*(\d+):", text, re.IGNORECASE)
    # instr = re.findall(r"\[INSTR\]\s*Step\s*(\d+):", text)
    # execs = re.findall(r"\[EXEC\]\s*Step\s*(\d+):", text)
    
    if not instr or not execs:
        return reward

    # ---- (A) Alternation check ----
    tags = re.findall(r"<\|(instruction|execution)\|>", text, re.IGNORECASE)
    # tags = re.findall(r"\[(INSTR|EXEC)\]", text)
    alternates = all(tags[i] != tags[i+1] for i in range(len(tags)-1))
    # if alternates:
    #     reward += 0.1

    # ---- (B) Step alignment ----
    matched = all(i == e for i, e in zip(instr, execs))
    # if matched:
    #     reward += 0.1

    # ---- (C) Boxed final answer ----
    boxed_final_answer = re.search(r"\\boxed\{.*?\}", text, re.DOTALL)
    # if boxed_final_answer:
    #     reward += 0.1

    # ---- (D) Valid hierarchy ----
    hierarchy = len(instr) == len(execs) and len(instr) > 0
    # if hierarchy:
    #     reward += 0.1
    
    if alternates and matched and boxed_final_answer and hierarchy:
        reward = 1.0

    return reward


def boxed_reward_fn_with_format_reward(
    model_response, accuracy_reward
):
    format_reward = hierarchy_format_reward(model_response)
    
    return {"formatted": format_reward}, accuracy_reward + format_reward
